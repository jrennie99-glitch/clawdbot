...
import { isPrivateIp, isBlockedHostname, generateSecureId, nowIso } from './utils.js';
...
const POLICY_RULES: PolicyRule[] = [
  ...
  },
  {
    ...
    condition: (ctx) => {
      ...
      const isOutsideWorkspace = path && !path.startsWith('/workspace') && !path.startsWith('./') && !path.includes('/tmp/');
  ...
  // ========== ALLOW (SAFE OPERATIONS) ==========
  {
    ...
    priority: 1000,
    condition: (ctx) => {
      const readOnlyTools = ['read', 'list', 'search', 'memory_search', 'memory_get', 'session_status'];
...
export function cleanupExpiredPreviews(): number {
  ...
  for (const [id, preview] of pendingPreviews) {
    if (now > preview.expiresAt || preview.status !== 'pending') {
      ...
      if (age > 3600000) { // 1 hour after expiry
